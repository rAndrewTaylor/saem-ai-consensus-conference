...
import { CheckCircle2, AlertCircle, ArrowRight, Loader2, Crown } from 'lucide-react';
...
export function LeadClaimPage() {
  ...
  useEffect(() => {
    ...
    api('/api/co-leads/claim', { params: { token } })
      .then((data) => {
        setInfo(data);
  ...
  return (
    <div className="flex min-h-[70vh] flex-col items-center justify-center bg-[#0A1628] px-4 py-16 sm:px-6">
      <div className="pointer-events-none fixed -top-32 left-1/2 h-[500px] w-[800px] -translate-x-1/2 rounded-full bg-gradient-to-b from-amber-500/15 to-transparent blur-3xl" />
            ...
            <Card>
              <CardContent className="flex flex-col items-center py-14 text-center">
                <motion.div
                  ...
                  className="flex h-14 w-14 items-center justify-center rounded-2xl bg-amber-500/15 text-amber-400"
                >
                ...
                {info.wg_number && (
                  <p className="mt-2 max-w-sm text-sm text-white/60">
                    You&apos;re signed in as a co-lead for
                    <Badge variant="warning" className="mx-1.5">
                      WG {info.wg_number}
                    </Badge>
                ...
                <p className="mt-3 max-w-sm text-xs text-white/40">
                  Bookmark this page &mdash; your invite link is also how you sign back in on other devices.
                </p>
